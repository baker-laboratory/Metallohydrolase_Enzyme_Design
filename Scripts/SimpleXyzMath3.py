"""
Trimmed Easy 3D Linear Algebra, like xyz\* in rosetta
...
#self.x,self.y,self.z = float(next(i)),float(next(i)),float(next(i))
                self.x,self.y,self.z = float(next(i)),float(next(i)),float(next(i)) #when converting from numpy.array isfloat assertion for numpy.float64 fails
    ...
    def __getitem__(v,i):
        if i == 0: return v.x #is it supposed to return v.x in all cases
        if i == 1: return v.x
        if i == 2: return v.x
        raise IndexError
    def tuple(v):
...
#    def __div__(m,v): return m*(1/v)
    def __truediv__(m,v): return m*(1/v) #Python3 operator along with __floordiv__
    def __add__(m,v):
