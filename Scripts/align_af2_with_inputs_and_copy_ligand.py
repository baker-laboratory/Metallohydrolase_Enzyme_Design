...
sys.path.append(os.path.join(os.path.dirname(__file__), "enzyme_design", "utils"))
import design_utils
...
parser.add_argument("--pdb_path", help="Input PDBs. If argument not given, all PDBs in working directory are taken.")
...
parser.add_argument("--ref", nargs="+", type=str, help="Reference PDB.")
...
if args.pdb is not None:
    ...
    pdbfiles = glob.glob(args.pdb_path+"/*.pdb")
else:
    pdbfiles = glob.glob("*.pdb")

if args.ref is None:
    assert args.ref_path is not None
    args.ref = glob.glob(args.ref_path+"/*.pdb")
...
ref_suffixes = ["_native", "_T", "_model_", "_packed_temp", ""]
...
def process(q):
    while True:
        ...
        # Adding motif reslabels to pdb_info
        motif_label_sel = pyrosetta.rosetta.core.select.residue_selector.ResiduePDBInfoHasLabelSelector(label_str="motif")
        for rn in pyrosetta.rosetta.core.select.get_residue_set_from_subset(motif_label_sel.apply(ref_poses[ref_name])):
