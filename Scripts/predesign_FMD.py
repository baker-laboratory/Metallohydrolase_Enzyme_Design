...
# FastMPNNdesign.py and Selectors.py sit at the top of the fastmpnndesign
# submodule. The old path (Scripts/enzyme_design/FastMPNNDesign) does not exist,
# so this import could never have succeeded.
_fmd = os.path.normpath(os.path.join(script_dir, "..", "Software", "fastmpnndesign"))
if not os.path.isdir(_fmd):
    raise ImportError(
        f"fastmpnndesign not found at {_fmd}.\n"
        "  Initialize the submodules:  git submodule update --init --recursive")
sys.path.append(_fmd)
