...
import align_pdbs  # from enzyme_design/utils, appended just above
...
def process(q):
    while True:
        ...
        ## prmsd average / min
        DF.at[i, f"prmsd_avr"] = _df.prmsd.mean()
        DF.at[i, f"prmsd_min"] = _df.prmsd.min()
        ...
        DF.at[i, f"prmsd_top{args.top}"] = _df.sort_values("prmsd", ascending=True).prmsd.iloc[:int(args.top)].mean()
        DF.at[i, f"rmsd_prmsd_top{args.top}"] = _df.sort_values("prmsd", ascending=True).rmsd_ligand.iloc[:int(args.top)].mean()
