...
def get_matcher_residues(filename):
    """
    Returns a dictionary with REMARK 666 contents
    Arguments:
        filename (str)
        filename (pyrosetta.rosetta.core.pose.Pose)
    """
...
def get_packer_layers(pose, ref_resno, cuts, target_atoms, do_not_design=None, allow_design=None, design_GP=False):
    ...
    Arguments:
        ...
        ref_resno (int) :: residue around which the layers are centered
        cuts (list) :: list of 4 floats defining CA distances from heavyatoms.
        ...
        allow_design (list) :: positions that are designable
...
def setup_fastrelax(sfx, crude=False, disable_min_resons=None):
    ...
    if disable_min_resons is not None:
        # asd = pyrosetta.rosetta.std.vector_std_pair_unsigned_long_unsigned_long_t()
        # for x in disable_min_resons:
        #   asd.append((x, x))
        # mm.set_ranges_unmodifiable(asd)
...
def find_clashes_between_target_and_sidechains(pose, target_resno, target_atoms=None, residues=None):
    ...
    for res in pose.residues:
        ...
        for ano in range(1, res.natoms()+1):
            if res.is_virtual(ano):
                continue
            if res.atom_is_hydrogen(ano):
                continue
            if min([(pose.residue(target_resno).xyz(ha)-res.xyz(ano)).norm() for ha in target_atoms]) < 2.5:
...
def match_sidechain_atoms(target, target_res_to_match,
    ...
    spliced_pose = Pose()
    append_subpose_to_pose(spliced_pose, target, 1, target.size() - 1, True)
    append_subpose_to_pose(spliced_pose, loop, 1, loop_length, True)
    ###############################################################

    return spliced_pose


def align_res_to_res(target_res, align_res, target_atom_list_to_match, align_res_atom_list_to_match):
    """
    Given a pose of the target+a residue, and a pose of a loop, 
    and which residue to match
    ...
    # make the rosetta object to hold xyz crds of the residue we want to align
    target_match_coords = xyzVector()
    align_coords = xyzVector()

    # add the coords of the residues to be aligned to their respctive holders
    for target_name, align_name in zip(target_atom_list_to_match, align_res_atom_list_to_match):
        target_match_coords.append(target_res.xyz(target_name))
        align_coords.append(align_res.xyz(align_name))

    # make the rotation matrix and pose center rosetta objects
    rotation_matrix = xyzMatrix_double_t()
    target_center = xyzVector_double_t()
    align_center = xyzVector_double_t()
    ...
    # making a temporary pose for the align res
    _res = Pose()
    _res.append_residue_by_jump(align_res, 0)
