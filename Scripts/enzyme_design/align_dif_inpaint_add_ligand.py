...
def main():
    ...
    parser.add_argument("--fix_motif", action="store_true", default=False, help="Fixes also motif residue rotamers. IN ADDITION to the catalytic residues.")
    ...
    def process(q, ref_poses):
        while True:
            ...
            for (template_i, target_i) in align_positions:
                # If not all sequence got redesigned then aligning to to positions that have retained their identity
                if not all([x == False for x in trb["inpaint_seq"]]):
                    if bb_inpaint is True and trb["inpaint_seq"][target_i] == False:
                        continue
                else:
                    # otherwise aligning to all contig positions that didn't get inpainted
                    if bb_inpaint is True and trb["inpaint_str"][target_i] == False:
                        continue
            ...
            """
            Fixing all other motif residues, if user requested
            """
            if args.fix_motif is True:
                for template_i, target_i in zip(trb["con_ref_idx0"], trb["con_hal_idx0"]):
                    if template_i+1 in matched_residues:
                        continue
                    if ref_pose.residue(template_i+1).name3() != pose2.residue(target_i+1).name3():
                        continue
                    if pose2.residue(target_i+1).name1() in "GPA":
                        continue
                    if ref_pose.residue(template_i+1).name3() == "HIS" and ref_pose.residue(template_i+1).name() != pose2.residue(target_i+1).name():
                        mutres = pyrosetta.rosetta.protocols.simple_moves.MutateResidue()
                        mutres.set_res_name(ref_pose.residue(template_i+1).name())
                        mutres.set_target(target_i+1)
                        mutres.apply(pose2)
                    for n in range(ref_pose.residue(template_i+1).nchi()):
                        pose2.residue(target_i+1).set_chi(n+1, ref_pose.residue(template_i+1).chi(n+1))

            # Adding motif residues also a REMARK PDBinfo-LABEL:
            for rn in trb["con_hal_idx0"]:
                if trb["inpaint_seq"][rn] == True:
                    if pose2.residue(rn+1).name3() in ["ALA"]:  # Hackily skipping ALA motifs - not safe
                        continue
    ...
    with open(args.outdir+"/rmsds.csv", "w") as file:
        file.write("rmsd,description\n")
        for k in rmsds.keys():
            file.write(f"{rmsds[k]:.3f},{k}\n")
