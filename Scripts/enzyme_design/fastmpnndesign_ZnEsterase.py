...
# Resolve from this file, not the caller's working directory -- these were
# cwd-relative and spelled "scripts" in lowercase, so they broke on any
# case-sensitive filesystem and from every directory but one.
from pathlib import Path as _Path
_HERE = _Path(__file__).resolve().parent
for _anc in _HERE.parents:
    if (_anc / "Software" / "fastmpnndesign").is_dir():
        sys.path.append(str(_anc / "Software" / "fastmpnndesign"))
        break
import FastMPNNdesign
from Selectors import SelectHBondsToResidue
import ScoringCalculators
sys.path.append(str(_HERE / "utils"))
import design_utils
