...
# Shared tool resolution, so this script and its caller agree on which Open
# Babel to use. See Scripts/env_config.py.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent / "Scripts"))
from env_config import resolve_obabel, ToolNotFoundError  # noqa: E402
...
# Path to the Open Babel executable. Resolved at startup by main() via
# resolve_obabel(); this default is what a direct `import` of this module sees
# before main() runs.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="OPTIONAL. Path to the `obabel` executable. Leave unset to use the "
             "one on $PATH (correct inside the `zinc_hydro` conda environment). "
             "Can also be set via $ZINC_HYDRO_OBABEL. theozyme_XYZ_to_PDB__MAIN.py "
             "passes its own resolved value through this flag.",
    )

    args = parser.parse_args()

    # Resolve Open Babel now so a missing install fails here with instructions,
    # rather than emitting an empty residue_map.txt several steps downstream.
    global OBABEL_PATH
    try:
        OBABEL_PATH = resolve_obabel(args.obabel_path)
    except ToolNotFoundError as exc:
        sys.exit(f"\n### OPEN BABEL NOT FOUND ###\n{exc}\n")
    print(f"### USING OPEN BABEL: {OBABEL_PATH} ###")
