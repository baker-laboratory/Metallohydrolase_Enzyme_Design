...
### TOOL RESOLUTION                                                        ###
##############################################################################
# No container is required. The STEP helpers need PyRosetta (STEP_2) and numpy
# (STEP_3), both of which ship in the `zinc_hydro` conda environment, so by
# default they run under the SAME interpreter that is running this script.
#
# A container remains available as an option: pass --apptainer /path/to.sif, or
# export ZINC_HYDRO_SIF.

sys.path.insert(0, str(SCRIPT_DIR.parent.parent / "Scripts"))
from env_config import resolve_runner, resolve_obabel, ToolNotFoundError  # noqa: E402

# Populated in __main__ (or by calling configure_tools()) before main() runs.
RUNNER: list = [sys.executable]   # argv prefix for the STEP helper scripts
OBABEL_PATH: str = "obabel"       # resolved Open Babel executable


def configure_tools(apptainer=None, obabel=None):
    """Resolve the interpreter/container and Open Babel once, up front.

    Doing this before any work starts means a bad path fails immediately with an
    actionable message, instead of producing a broken shell command whose real
    symptom appears several steps later as a missing residue_map.txt.
    """
    global RUNNER, OBABEL_PATH
    RUNNER = resolve_runner(apptainer)
    OBABEL_PATH = resolve_obabel(obabel)
    print("### TOOL CONFIGURATION ###")
    print(f"   STEP helper runner : {' '.join(RUNNER)}")
    print(f"   open babel         : {OBABEL_PATH}\n")


def _run(argv, what):
    """Run a subprocess and fail loudly if it does not succeed.

    Checking the return code here keeps a failure at the step that caused it,
    instead of surfacing later as a missing intermediate file.
    """
    ...
    if result.returncode != 0:
        raise RuntimeError(
            ...
            f"  Command: {' '.join(argv)}\n"
            f"  Re-run with --KEEP_temp_smiles_and_res_pdbs_for_debug and "
            f"--KEEP_temp_residue_and_temp_ligand_files_for_debug to inspect "
...
def main(
    ...
    _run([OBABEL_PATH, "-ixyz", temp_residue_xyz_file, "--separate", "-opdb", "-O", residue_pdb_file],
         "Open Babel conversion of the residue XYZ")
    ...
    _run([OBABEL_PATH, "-ixyz", temp_ligand_xyz_file, "-opdb", "-O", ligand_pdb_file],
         "Open Babel conversion of the ligand XYZ")
    ...
    identify_cmd = RUNNER + [
        IDENTIFY_RESIDUES_SCRIPT,
        "-input_pdb", residue_pdb_file,
        # STEP_1 shells out to Open Babel itself, so it needs the same resolved
        # executable this script uses -- it must not fall back to a hardcoded path.
        "-obabel_path", OBABEL_PATH,
    ]
    if DO_NOT_pass_tip_atom_residues_3letter_to_help_identifier:
        identify_cmd += ["-tip_atom_residues_3letter", *tip_atom_residues_3letter]
    _run(identify_cmd, "STEP_1 residue identification")

    print("PASSED RESIDUE IDENTIFICATION\n")

    # Read mapping from residue_map.txt
    if not os.path.exists("residue_map.txt"):
        raise RuntimeError(
            "STEP_1 reported success but did not write residue_map.txt in "
            f"{os.getcwd()}.\n"
            "  This usually means Open Babel produced no SMILES for one or more "
            "residue fragments.\n"
            "  Re-run with --KEEP_temp_smiles_and_res_pdbs_for_debug and check "
            "the pdb*_TEMP.pdb / *_TEMP_smiles.smi files."
        )
    ...
    _run(RUNNER + [BUILD_FROM_TIPS_SCRIPT, "-input_pdb", residue_pdb_file],
         "STEP_2 Rosetta residue construction")
    ...
    for input_file, rosetta_file in pairs:
        command = RUNNER + [
            OPTIMAL_SUPERIMPOSE_SCRIPT,
            "-i", input_file,
            "-r", rosetta_file,
            "-o", "aligned_rosetta.pdb",
            "--omit_backbone_but_keep_CA",
        ]
        _run(command, f"STEP_3 superposition of {rosetta_file} onto {input_file}")
        print(f"\n### EXECUTED: {' '.join(str(c) for c in command)} ###")
    ...
    command = RUNNER + [
        INSTALL_MISSING_ATOMS_SCRIPT,
        "-aligned_rosetta_pdbs_for_residues", *aligned_rosetta_files,
        "-dft_tip_atoms_of_residues_only", residue_pdb_file,
        "-output_pdb", final_output_pdb,
    ]
    _run(command, "STEP_4 missing-atom installation")
    print(f"\n### EXECUTED: {' '.join(str(c) for c in command)} ###")
    ...
    if ligand_atom_for_close_proximity_to_OE2glu_and_OD2asp:
        command = RUNNER + [
            STANDARDIZE_GLU_ASP_SCRIPT,
            "--input_pdb", output_pdb,
            "--ligand_code", lig_3letter_code,
            "--ligand_atom_for_close_proximity_to_OE2glu_and_OD2asp",
            ligand_atom_for_close_proximity_to_OE2glu_and_OD2asp,
        ]
        print(f"\n### EXECUTING STANDARDIZATION: {' '.join(str(c) for c in command)}")
        _run(command, "STEP_5 GLU/ASP tip-oxygen standardization")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--apptainer",
        default=None,
        help="OPTIONAL. Interpreter used to run the STEP_* helper scripts. "
             "Leave unset (the default) to use the interpreter running this "
             "script -- correct inside the `zinc_hydro` conda environment, which "
             "already provides PyRosetta. Pass a path to an Apptainer/Singularity "
             "image only if you want to run the helpers inside a container; build "
             "one with `apptainer build zinc_hydro.sif Environment/zinc_hydro.def`. "
             "Can also be set via $ZINC_HYDRO_SIF.",
    )
    parser.add_argument(
        "--obabel_path",
        default=None,
        help="OPTIONAL. Path to the `obabel` executable. Leave unset to use the "
             "one on $PATH (correct inside the conda environment). Can also be "
             "set via $ZINC_HYDRO_OBABEL.",
    )

    args = parser.parse_args()

    # Resolve tools up front so a bad path fails here, with instructions,
    # rather than several steps later as a missing intermediate file.
    try:
        configure_tools(apptainer=args.apptainer, obabel=args.obabel_path)
